...
std::tuple<bool, float, float, float, float> Ray::ray_triangle_intersection(const glm::vec3 &v0, const glm::vec3 &v1, const glm::vec3 &v2) const {
    ...
    if (area2 < eps) return std::make_tuple(false, 0, 0, 0, 0);
    normal = normal / area2;
    ...
    auto get_param = [&](const glm::vec3 &r, const glm::vec3 &s) -> float {
        ...
        if (glm::dot(normal, c) < 0) return -1;
        return area / area2;
    };
